...
<article id="post-<?php the_ID(); ?>" <?php post_class($furnilife_postclass); ?>> 
	...
	<?php 
		if ( is_single() ) { ?>
			...
			<div class="player"><?php echo do_shortcode(get_post_meta( $post->ID, '_furnilife_post_intro', true )); ?></div>
		<?php }
	?>
	<?php if ( !is_single() ) { ?>
		<?php if ( has_post_thumbnail() ) { ?>
		<div class="post-thumbnail">
			<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail($furnilife_postthumb); ?></a>
			...
			<?php if ( is_single() ) : ?>
				...
				<div class="author-info">
					...
					<div class="author-description">
						<h2><?php esc_html_e( 'About the Author:', 'furnilife'); printf( '<a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'" rel="author">%s</a>' , get_the_author()); ?></h2>
