...
$furnilife_products_count = Furnilife_Class::furnilife_products_count();
...
$furnilife_shopclass = Furnilife_Class::furnilife_shop_class('');

$colwidth = 4;

if($furnilife_shopclass=='shop-fullwidth') {
	if(isset($furnilife_opt)){
		$woocommerce_loop['columns'] = $furnilife_opt['product_per_row_fw'];
	...
	if(isset($furnilife_opt)){
		$woocommerce_loop['columns'] = $furnilife_opt['product_per_row'];
		if($woocommerce_loop['columns'] > 0){
			$colwidth = round(12/$woocommerce_loop['columns']);
		}
	}
...
<?php
if ($furnilife_viewmode=='grid-view') {
...
<div <?php post_class( $classes ); ?>>
	<div class="product-wrapper"> 
		<div class="list-col4 <?php if($furnilife_viewmode=='list-view'){ echo ' col-xs-12 col-sm-4';} ?>">
			<div class="product-image">
				<?php do_action( 'woocommerce_before_shop_loop_item' ); ?>
					...
					if(isset($furnilife_opt['second_image'])){
						if($furnilife_opt['second_image']){
				...
				<?php if ( $product->is_on_sale() ) : ?>
					<?php echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale"><span class="sale-bg"></span><span class="sale-text">' . esc_html__( 'Sale', 'furnilife' ) . '</span></span>', $post, $product ); ?>
				<?php endif; ?>  
		...
		<div class="list-col8 <?php if($furnilife_viewmode=='list-view'){ echo ' col-xs-12 col-sm-8';} ?>">
			<div class="gridview">
				...
				<div class="box-hover"> 
					<ul class="add-to-links"> 
						...
						<?php if ( isset($furnilife_opt['quickview']) && $furnilife_opt['quickview'] ) { ?>
							<li class="quickview-inner">
								<div class="quickviewbtn">
									<a class="detail-link quickview" data-quick-id="<?php the_ID();?>" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php echo esc_html($furnilife_opt['detail_link_text']);?></a>
								</div>
							</li>
						<?php } ?>   
					</ul> 
...
if ($furnilife_viewmode=='grid-view') {  
	$itemindex = 12/ $colwidth;   
	if ( ( 0 == ( $woocommerce_loop['loop'] ) % $itemindex  ) || ($furnilife_products_count == $woocommerce_loop['loop']) ) {
		echo '</div>';
	}   
}?> 